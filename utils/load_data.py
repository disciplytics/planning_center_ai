...
def load_data(pco):
  ...
  async def fetch_people_data():
    try:
      ...
      return pd.json_normalize(people_data_df['relationships.households.data'])#people_include_df[people_include_df['type'] == 'PhoneNumber'].dropna(axis=1, how='all').drop_duplicates()
